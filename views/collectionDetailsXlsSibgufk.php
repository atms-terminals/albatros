...
if ($opers) {
    $xls->setActiveSheetIndex(0)
        ...
        ->setCellValue($col++.$row, 'id контрагента')
        ->setCellValue($col++.$row, 'ФИО')
        ->setCellValue($col++.$row, 'Паспорт')
    ...
    foreach ($opers as $item) {
        ...
        $xls->setActiveSheetIndex(0)
            ...
            ->setCellValue($col++.$row, $item['service'])    // C
            ->setCellValue($col++.$row, $item['id_contragent'])      // D
            ->setCellValue($col++.$row, $item['contragent'])   // E
            ->setCellValue($col++.$row, $item['passport'])    // F
            ->setCellValue($col++.$row, $item['amount']);     // H
        $row++;
    }
